...
def resize_img(im):
    ...
    # 补hui色边
    new_im = ImageOps.expand(resized_im, border=(
        left_pad, top_pad, right_pad, bottom_pad), fill=(114, 114, 114))
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            conf, j = x[:, 5:mi].max(1, keepdim=True)
            x = torch.cat((box, x[..., 4:5], j.float(), mask), 1)[
                conf.view(-1) > 0]
        # Filter by class
        if classes is not None:
            x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]
...
class YOLOv5:
    def __init__(self, model_path, image_queue, infer_queue, show_queue):
        self.label = ['Monster', 'Monster_ds', 'Monster_szt', 'card', 'equipment', 'go', 'hero',
                      'map', 'opendoor_d', 'opendoor_l', 'opendoor_r', 'opendoor_u', 'pet', 'Diamond']
    ...
    def thread(self):
        ...
        while True:
            ...
            image_array = np.expand_dims(
                image_array, axis=0).astype(np.float32)
