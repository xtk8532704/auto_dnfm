...
if __name__ == '__main__':
    ...
    yolo = YOLOv5(os.path.join(current_dir, "./utils/dnfm.onnx"),
                  image_queue, infer_queue, show_queue)
    control = GameControl(client, os.path.join(
    ...
    while True:
        ...
        for boxs in result:
            ...
            cv2.putText(image, "{:.2f}".format(conf), (int(x1), int(
                y1-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            cv2.putText(image, yolo.label[int(label)], (int(x1), int(
                y1-30)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        image = cv2.resize(
            image, (1800, int(image.shape[0]*1800/image.shape[1])))
        # 创建按钮区域
        button_panel_width = 100
        button_panel = np.zeros(
            (image.shape[0], button_panel_width, 3), dtype=np.uint8)
        # 按钮属性
        button_height = 50
        button_gap = 10
        ...
        def draw_buttons(panel):
            for i, label in enumerate(buttons):
                ...
                y2 = y1 + button_height
                cv2.rectangle(panel, (10, y1),
                              (button_panel_width - 10, y2), button_color, -1)
                cv2.putText(panel, label, (20, y1 + 35),
        ...
        def on_mouse(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                if x > image.shape[1]:  # 检查是否点击在按钮区域
                    ...
                    control.click(
                        x / image.shape[1]*2400, y / image.shape[0]*1080)
